...
// ── Constants ──────────────────────────────────────────────────────────────
// NAV_LS: only stores screen/region/zone — NEVER coins, caught, pokeballs.
// All game state comes from the server. Clearing localStorage loses nothing important.
const NAV_LS  = 'druygon-nav-v2';
const SLOT    = 1;   // active player slot (selector UI is Phase D)
...
function saveNav(screen, region, zone) {
  try { localStorage.setItem(NAV_LS, JSON.stringify({ screen, region, zone })); } catch { /* storage full */ }
}
...
function App() {
  ...
  const [caught,    setCaught]    = React.useState([]);   // [{dex, zoneId, caughtAt}]
  const [progress,  setProgress]  = React.useState([]);   // [{zoneId, status, clearedAt}]

  // Celebration overlay
  const [celeb, setCeleb] = React.useState(null);

  // Load player from server on mount
  React.useEffect(() => {
    apiGet(`/api/player/${SLOT}`)
      ...
      .catch(err => {
        ...
        setPlayerReady(true);   // still render — offline mode
        setPlayerErr(err.message);
      });
  }, []);

  // Navigation helper — always saves to localStorage
  ...
  // Called by Catch screen when a Pokémon is successfully thrown at
  const onCaught = async (mon, ball) => {
    ...
    // Persist to server (non-blocking — failure doesn't crash UI)
    try {
      ...
      // State remains in optimistic form; will reconcile on next page load
    }
  };

  const closeCeleb = () => { setCeleb(null); go('map', region); };

  // Provide caught dex array for legacy components that expect [dex, dex, ...]
  const caughtDex = caught.map(c => c.dex);

  // Pokeball list for Catch screen (injects _zoneId + _coinAward into each ball)
  const currentZoneId = (() => {
    ...
    return (
      <div className="device">
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', height: '100%', flexDirection: 'column', gap: 12, color: 'var(--text-secondary)' }}>
          ...
          <div style={{ fontFamily: 'var(--font-display)', fontSize: 13 }}>Loading Dru's data…</div>
        </div>
      </div>
    );
  }

  // REGIONS may be null while content is still loading (useContent hook handles this per-screen)
  const r = window.REGIONS && window.REGIONS[region];

  let header, content, showNav = true;

  if (screen === 'home') {
    header  = <Header region={region} coins={profile.coins} />;
    content = <Home go={go} caught={caughtDex} coins={profile.coins} />;
    ...
    <React.Fragment>
      ...
      {playerErr && (
        <div style={{ position: 'fixed', bottom: 8, left: '50%', transform: 'translateX(-50%)', background: '#2a1a1a', color: '#f87171', padding: '6px 14px', borderRadius: 8, fontSize: 11, zIndex: 9999, maxWidth: 300, textAlign: 'center' }}>
          Offline mode — progress may not save. ({playerErr})
        </div>
      )}
    </React.Fragment>
  );
