...
type OutputState =
  | { kind: 'initial' | 'saved' | 'offlineSafe' | 'running' | 'noOutput' | 'offlineRetry' | 'reset' }
...
const COPY = {
  en: {
    ...
    backMap: 'Back to the learning map', missionOne: 'MISSION 1', firstCommand: 'First Command', runnerReady: 'Code runner ready', runnerChecking: 'Checking runner…', runnerOffline: 'Code runner asleep',
    storyLabel: 'MISSION STORY', storyTitle: 'Nara wants to move', story: 'Nara sees a star ahead, but she does not know the command to move yet.',
    ...
    hints: ['Start with the command that moves Nara one step.', 'You need one forward command. Then run it to see what happens.', 'Find the block or line that means “move forward one step”. Leave everything else unchanged for now.'],
    codeArea: 'Code area', draftSafe: 'Draft saved automatically', codeLabel: 'Mission code', run: 'Run', reset: 'Reset', save: 'Save',
    starterCode: '# Help Nara move forward one step\nmove(1)',
    initialOutput: 'Press Run when the code runner is connected.', savedOutput: 'Draft saved on this device.', offlineSafeOutput: 'The code runner is asleep. Your code is safe and saved.',
    runningOutput: 'The code runner is testing your code…', noOutput: 'Code finished with no output.', offlineRetryOutput: 'The code runner is asleep. Try again later!', resetOutput: 'Code reset to the start of the mission.',
  ...
  id: {
    ...
    backMap: 'Kembali ke peta', missionOne: 'MISI 1', firstCommand: 'Perintah Pertama', runnerReady: 'Robot pelari siap', runnerChecking: 'Memeriksa robot…', runnerOffline: 'Robot pelari tidur',
    storyLabel: 'CERITA MISI', storyTitle: 'Nara ingin maju', story: 'Nara melihat bintang di depan, tapi ia belum tahu perintah untuk bergerak.',
    ...
    hints: ['Mulai dari perintah yang membuat Nara bergerak satu langkah.', 'Kamu butuh satu perintah maju, lalu jalankan untuk melihat hasilnya.', 'Cari blok atau baris yang bermakna “maju satu langkah”. Jangan ubah bagian lain dulu.'],
    codeArea: 'Area kode', draftSafe: 'Draft otomatis aman', codeLabel: 'Kode misi', run: 'Jalankan', reset: 'Reset', save: 'Simpan',
    starterCode: '# Bantu Nara bergerak satu langkah\nmaju(1)',
    initialOutput: 'Tekan Jalankan saat robot pelari kode sudah terhubung.', savedOutput: 'Draft tersimpan di perangkat ini.', offlineSafeOutput: 'Robot pelari kode lagi tidur. Kode kamu tetap aman dan tersimpan.',
    runningOutput: 'Robot pelari sedang mencoba kode…', noOutput: 'Kode selesai tanpa output.', offlineRetryOutput: 'Robot pelari kode lagi tidur. Coba lagi nanti ya!', resetOutput: 'Kode dikembalikan ke awal misi.',
...
function outputText(language: Language, output: OutputState) {
  ...
  const messages = {
    initial: t.initialOutput, saved: t.savedOutput, offlineSafe: t.offlineSafeOutput, running: t.runningOutput,
    noOutput: t.noOutput, offlineRetry: t.offlineRetryOutput, reset: t.resetOutput,
  };
...
function App() {
  ...
  const [code, setCode] = React.useState(() => safeLocalGet(`drucode-draft-${slot}-lesson-1`) ?? t.starterCode);
  const [hintLevel, setHintLevel] = React.useState(0);
  const [output, setOutput] = React.useState<OutputState>({ kind: 'initial' });
  const [runnerState, setRunnerState] = React.useState<'checking' | 'ready' | 'offline'>('checking');
  ...
  React.useEffect(() => {
    const controller = new AbortController();
    fetch('/api/code/health', { signal: controller.signal })
      .then((response) => {
        if (!response.ok) throw new Error('offline');
        setRunnerState('ready');
      })
      .catch((error: Error) => {
        if (error.name !== 'AbortError') setRunnerState('offline');
      });
    return () => controller.abort();
  }, []);

  const saveDraft = React.useCallback(() => {
    safeLocalSet(`drucode-draft-${slot}-lesson-1`, code);
    setOutput({ kind: 'saved' });
  }, [code, slot]);

  const runCode = React.useCallback(async () => {
    if (runnerState !== 'ready') {
      setOutput({ kind: 'offlineSafe' });
      return;
    }
    setOutput({ kind: 'running' });
    try {
      const response = await fetch('/api/code/workspace/run', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ slot, lessonId: 1, language: 'blockly', code }),
      });
      const result = await response.json();
      if (!response.ok) throw new Error(result?.error || 'run failed');
      setOutput(result.stdout ? { kind: 'raw', value: result.stdout } : { kind: 'noOutput' });
    } catch {
      setRunnerState('offline');
      setOutput({ kind: 'offlineRetry' });
    }
  }, [code, runnerState, slot]);
...
type WorkspaceProps = {
  ...
  hintLevel: number;
  output: string;
  runnerState: 'checking' | 'ready' | 'offline';
  onBack: () => void;
...
function Workspace({ code, language, hintLevel, output, runnerState, onBack, onCodeChange, onHint, onReset, onRun, onSave }: WorkspaceProps) {
  const t = COPY[language];
  return (
    <div className="workspace">
      <div className="workspace-heading">
        ...
        <span className={`runner-chip ${runnerState}`}>
          <i /> {runnerState === 'ready' ? t.runnerReady : runnerState === 'checking' ? t.runnerChecking : t.runnerOffline}
        </span>
      </div>
      ...
      <div className="workspace-grid">
        ...
        <section className="code-zone" aria-label={t.codeArea}>
          ...
          <textarea id="code-editor" value={code} onChange={(event) => onCodeChange(event.target.value)} spellCheck={false} />
